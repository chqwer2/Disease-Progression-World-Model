...
def load_if(checkpoints_path: Optional[str], network: nn.Module) -> nn.Module:
    ...
    if checkpoints_path is not None:
        if not os.path.exists(checkpoints_path):
            print("Weights invalid: ", os.path.exists(checkpoints_path))
            return network
        ...
        print("Load successful:", checkpoints_path)
...
def init_autoencoder(checkpoints_path: Optional[str] = None) -> nn.Module:
    ...
    autoencoder = AutoencoderKL(spatial_dims=3,
                                ...
                                out_channels=1,
                                latent_channels=3,
                                channels=(64, 128, 128, 128),  # (64, 128, 128, 128)
                                num_res_blocks=2,
                                norm_num_groups=32,
                                norm_eps=1e-06,
...
def init_mamba_diffusion(checkpoints_path: Optional[str] = None) -> nn.Module:
    """
    Load the UNet from the diffusion model (pretrained if `checkpoints_path` points to previous params).
    ...
    Returns:
        nn.Module: the UNet
    """
    from .mamba import MambaUnet

    # print("Init init_mamba_diffusion")

    latent_diffusion = MambaUnet(in_chans=4,
                                 out_chans=4,
                                 depths=[4, 4, 4, 4],  # [3, 3, 3, 3],
                                 feat_size=[48, 144, 384, 528],  # 48
                                 drop_path_rate=0,
...
def init_mamba_controlnet(checkpoints_path: Optional[str] = None) -> nn.Module:
    """
    Load the ControlNet (pretrained if `checkpoints_path` points to previous params).
    ...
    Returns:
        nn.Module: the ControlNet
    """
    from .control_mamba import MambaControl

    controlnet = MambaControl(in_chans=4,
                              out_chans=4,
                              depths=[4, 4, 4, 4],  # [3, 3, 3, 3],
                              feat_size=[48, 144, 384, 528],  # 48
                              drop_path_rate=0,

...
def sample_using_diffusion(
    ...
    # z = trans.to_vae_latent_trick( z.squeeze(0).cpu() )
    with autocast(enabled=True):
        x = autoencoder.decode( z ).to(device)
    # x = trans.to_mni_space_1p5mm_trick( x.squeeze(0).cpu() ).squeeze(0)
    return x[0,0,:,:].cpu()
...
def sample_using_controlnet_and_z(
    ...
    followup_z             = followup_z.unsqueeze(0).to(device)
    ...
    z = (z / scale_factor).sum(axis=0, keepdim=True) / average_over_n

    def decode_with_tiling(autoencoder, z, roi_size=(8, 16, 16), overlap=0.25, device="cuda"):
        """
        Decode a latent tensor z in tiles along D, H, W to avoid OOM.
        Args:
            autoencoder: your trained autoencoder
            z: latent tensor [B, C, D, H, W]
            roi_size: size of tile in (D, H, W)
            overlap: fraction overlap between tiles (for blending)
        Returns:
            Reconstructed full volume [B, C, D, H, W]
        """
        from monai.inferers import sliding_window_inference

        # wrapper to decode a patch
        def _decode_fn(latent_patch):
            with torch.cuda.amp.autocast():
                return autoencoder.decode_stage_2_outputs(latent_patch)

        # use MONAI's built-in sliding window inference
        recon = sliding_window_inference(
            inputs=z,
            roi_size=roi_size,
            sw_batch_size=1,
            predictor=_decode_fn,
            overlap=overlap,
    ...
    z = z.to(device, dtype=torch.float16).detach()
    starting_z = (starting_z / scale_factor).to(device, dtype=torch.float16).detach()
    followup_z = (followup_z / scale_factor).to(device, dtype=torch.float16).detach()

    print("")
    x =  autoencoder.decode_stage_2_outputs(z)  #decode_with_tiling(autoencoder, z, roi_size=(8, 10, 16), overlap=0.25, device=device)
    start_x = autoencoder.decode_stage_2_outputs(starting_z) #decode_with_tiling(autoencoder, starting_z, roi_size=(8, 10, 16), overlap=0.25, device=device)
    follow_x = autoencoder.decode_stage_2_outputs(followup_z)  #decode_with_tiling(autoencoder, followup_z, roi_size=(8, 10, 16), overlap=0.25, device=device)

    # ---- original
    x = x.float().detach().cpu().numpy()[0,0]  # 128, 160, 128
    start_x = start_x.float().detach().cpu().numpy()[0,0]
    follow_x = follow_x.float().detach().cpu().numpy()[0,0]

    return x, start_x, follow_x


@torch.no_grad()
def sample_using_controlnet_and_z_batch_test(
        autoencoder: nn.Module,
        diffusion: nn.Module,
        controlnet: nn.Module,
        starting_z: torch.Tensor,
    ...
    z = torch.randn(*starting_z.shape).to(device)

    z = z * ratio + starting_z * (1 - ratio)  # mixing the starting latent with the noise
    ...
    for t in progress_bar:
        with torch.no_grad():
            with autocast(enabled=True):
                ...
                # for i in range(len(down_h)):
                #     print(f"down_h[{i}] =", torch.mean(torch.tensor(down_h[i])))
                #
                # print("mid_h =", torch.mean(mid_h))
                ...
                # print("clean noise_pred=", torch.mean(noise_pred))
                ...
                # print("fixed noise_pred=", torch.mean(noise_pred))

                # the scheduler applies the formula to get the
                # denoised step z_{t-1} from z_t and the predicted noise

                new_z, _ = scheduler.step(noise_pred, t, z)
                if not torch.isnan(new_z).any():
                    z = new_z
                else:
                    break
    ...
    # Here we conclude Latent Average Stabilization by averaging
    # m different latents from m different samplings.
    z = (z / scale_factor) / average_over_n  # .sum(axis=0)
    # z = trans.to_vae_latent_trick(z.squeeze(0).cpu())
